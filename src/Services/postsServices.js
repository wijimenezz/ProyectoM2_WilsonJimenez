...
export const getPostByAuthorIdService = async (authorId)=>{
    const { rows } = await pool.query("SELECT posts.*, authors.name, authors.email FROM posts JOIN authors ON posts.author_id = authors.id WHERE authors.id= $1",[authorId]);
    return(rows)
...
export const createPostService = async (datos)=>{
         ...
         const { rows } = await pool.query("INSERT INTO posts (title, content, author_id, published) VALUES ($1,$2,$3,$4) RETURNING *", 
...
export const updatePostService = async (id,datos) =>{
    const { rows } = await pool.query("UPDATE posts SET tittle = $1, content = $2 WHERE id = $3 RETURNING *", [datos.title, datos.content, id]);
    return (rows[0])
}
